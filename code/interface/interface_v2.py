...
# Versão final da interface!
    ...
    # Mas... é preciso carregar num botão para verificar serial input
        # (assim, evitamos que a aplicação esteja constantemente a atualizar)
...
import pandas as pd
from dash import Dash, dcc, html, Input, Output, callback_context, no_update  # Import no_update from dash
...
def create_scattermapbox_trace(df, custom_colorscale):
    return go.Scattermapbox(
        ...
        text=[f'Intensity: {intensity:.2f} dB, Button {i + 1}' for i, intensity in enumerate(df['Intensity_dB'])],  # Display intensity and button ID
...
def create_layout(df, zoom=12, center_lat=None, center_lon=None):
    if center_lat is None or center_lon is None:
        # Set default center coordinates here
        center_lat = 38.75
        center_lon = -9.15
    return go.Layout(
        mapbox=dict(
...
def register_callbacks(app):
    ...
    def update_graph_live(recenter_clicks, start_data_clicks, relayout_data):
        ...
        if trigger_id == 'recenter-button.n_clicks' or trigger_id is None:
            ...
            # Simulate click on Recenter button when app initializes
            recenter_clicks = 1
            start_data_clicks = None
